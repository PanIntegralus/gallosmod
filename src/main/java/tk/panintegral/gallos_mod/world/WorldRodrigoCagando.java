...
package tk.panintegral.gallos_mod.world;

import tk.panintegral.gallos_mod.item.ItemRodrigoCagando;
import tk.panintegral.gallos_mod.block.BlockPenisu;
import tk.panintegral.gallos_mod.block.BlockCarlingTable;
...
public class WorldRodrigoCagando extends ElementsGallosModMod.ModElement {
	public static int DIMID = 3;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("gallos_mod:rodrigo_cagando_portal")
	public static final BlockCustomPortal portal = null;
	public static DimensionType dtype;
	public WorldRodrigoCagando(ElementsGallosModMod instance) {
		super(instance, 47);
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ItemRodrigoCagando().setUnlocalizedName("rodrigo_cagando").setRegistryName("rodrigo_cagando"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Dimension ID for dimension rodrigo_cagando is already registered. Falling back to ID: " + DIMID);
		}
		dtype = DimensionType.register("rodrigo_cagando", "_rodrigo_cagando", DIMID, WorldProviderMod.class, true);
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(ItemRodrigoCagando.block, 0, new ModelResourceLocation("gallos_mod:rodrigo_cagando", "inventory"));
	}
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							this.world.setBlockState(new BlockPos(k9, k10, k11),
									flag ? BlockCarlingTable.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12),
								flag1 ? BlockCarlingTable.block.getDefaultState().getBlock().getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setUnlocalizedName("rodrigo_cagando_portal");
			setRegistryName("rodrigo_cagando_portal");
			setLightLevel(1F);
		}
		...
		public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.spawnParticle(EnumParticleTypes.TOTEM, px, py, pz, vx, vy, vz);
			}
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != BlockCarlingTable.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == BlockCarlingTable.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != BlockCarlingTable.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != BlockCarlingTable.block
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		private static final IBlockState STONE = Blocks.COAL_BLOCK.getDefaultState();
		private static final IBlockState STONE2 = Blocks.COAL_BLOCK.getDefaultState();
		private static final IBlockState FLUID = BlockPenisu.block.getDefaultState();
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		private Biome[] allowedBiomes = {Biome.REGISTRY.getObject(new ResourceLocation("ocean")),};
